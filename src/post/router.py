...
router = APIRouter(
    prefix="/post",
    tags=["posts"],
    dependencies=[Depends(require_valid_access_token)]
)
...
@router.get("/", description="Get all posts", response_model=list[PostResponseSchema] | None)
...
async def delete_a_post(
    ...
    Args:
        use_case (DeleteAPost): The use case instance for deleting a post.
